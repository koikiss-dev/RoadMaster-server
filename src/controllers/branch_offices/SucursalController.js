import { createSucursalcleShema } from "./dto/create_sucursal.js";
import { query } from "../../utils/query.js";
import { updateSucursalShema } from "./dto/update_sucursal.js";
...
class SucursalController extends ControllerBaseModel {
  ...
  static async createRegister(req, res) {
    const body = req.body;
    try {
      const { error, value } = createSucursalcleShema.validate(body);
      if (error) {
        return JoiError(error, res);
      }

      const { PV_NOM_SUCURSAL, PI_COD_GERENTE, PI_COD_UBICACION } = value;
  ...
  static async updateRegister(req, res) {
    const body = req.body;
    try {
      const { error, value } = updateSucursalShema.validate(body);
      if (error) {
        return JoiError(error, res);
      }
