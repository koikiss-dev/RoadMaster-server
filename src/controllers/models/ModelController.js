...
class ModelController extends ControllerBaseMode1 {
  ...
  static async createRegister(req, res) {
    ...
    try {
      ...
      const { PV_NOM_MODELO } = value;

      const sqlModel = "CALL INS_MODELO( ?)";

      const resultsModel = await query(sqlModel, [PV_NOM_MODELO]);
      res.status(201).json(resultsModel);
    } catch (error) {
      res.status(500).json({
        code: res.statusCode,
  ...
  static async updateRegister(req, res) {
    ...
    try {
      ...
      // extract values from the validated object
      const { PI_COD_MODELO, PV_NOM_MODELO } = value;

      const sql = "CALL UPD_MODELO(?, ?)";
      const results = await query(sql, [PI_COD_MODELO, PV_NOM_MODELO]);
      res.status(200).json(results);
    } catch (error) {
      res.status(500).json({
        code: res.statusCode,
